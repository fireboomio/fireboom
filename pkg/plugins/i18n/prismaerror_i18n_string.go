...
const (
	...
	_PrismaError_ZhCn_name_2 = "列{column_name}提供的值太长在where条件({model_name}.{argument_name} = {argument_value})中搜索的记录不存在约束{target}出现重复数据字段{field_name}上的外键约束失败数据库上的约束失败: {database_error}存储在数据库中字段{field_name}的值{field_value}对字段类型无效为{model_name}字段{field_name}提供的值{field_value}无效数据验证错误: {database_error}未能在{query_position}解析查询: {query_parsing_error}||kind=RequiredArgumentMissing<?>在{selectionPath}.{argumentPath}缺少一个必需值||kind=EmptySelection<?>在{selectionPath}({outputType.name})上期望至少有1个字段存在，结果是0||kind=InvalidArgumentType<?>在{selectionPath}.{argumentPath}出现无效的参数类型{inferredType},{argument.name}应该是以下类型之一{argument.typeNames}||kind=InvalidArgumentValue<?>在{selectionPath}.{argumentPath}出现无效的参数值,{value}不是有效的{argument.typeNames}||kind=SomeFieldsMissing<?>参数{selectionPath}.{argumentPath}缺失(约束{constraints})||kind=TooManyFieldsGiven<?>参数{selectionPath}.{argumentPath}给定过多(约束{constraints})||kind=SelectionSetOnScalar<?>无法在{selectionPath}中的标量字段{fieldName}上进行选择||kind=Union<?>无法将输入值与该字段允许的任何输入类型匹配。解析错误: <each>errors||kind=UnknownArgument<?>参数{selectionPath}.{argumentPath}类型不存在||kind=UnknownInputField<?>输入{selectionPath}.{argumentPath}(inputType.name)不存在||kind=UnknownSelectionField<?>字段{selectionPath}.{outputType.name}不存在||kind=ValueTooLarge<?>无法将参数{selectionPath}.{argumentPath}值{value}转换为64位有符号整数。考虑使用BigInt原始查询失败。代码: {code}。消息: {message}{constraint}上的空约束违规在{selectionPath}.{argumentPath}缺少一个必需值缺少{object_name}上字段{field_name}所需的参数{argument_name}您试图进行的更改将违反{model_a_name}和{model_b_name}模型之间的必要关系'{relation_name}'找不到相关记录。{details}查询解释错误。{details}{parent_name}和{child_name}模型之间的关系{relation_name}的记录没有连接找不到所需的连接记录。{details}输入错误。{details}该类型的值超出范围。{details}当前数据库中不存在表{table}当前数据库中不存在{column}列不一致的列数据: {message}从连接池中获取新连接的已超时(当前连接池超时: {timeout}s，连接已用: {connection_used}，连接限制: {connection_limit})。(更多信息: http://pris.ly/d/connection-pool)依赖一个或多个的记录未找到。{cause}当前数据库提供程序不支持查询使用的功能: {feature}查询执行期间数据库出现多个错误: {errors}事务API错误: {error}查询参数限制超过错误: {message}找不到用于搜索的全文索引，请尝试在您的模式中添加@@fulltext（[Fields...]）Prisma需要执行事务，这需要您的MongoDB服务器作为副本集运行。查看详情: https://pris.ly/d/mongodb-replica-set"
...
var (
	...
	_PrismaError_ZhCn_index_2 = [...]uint16{0, 34, 124, 156, 198, 243, 322, 386, 422, 482, 1736, 1791, 1824, 1878, 1948, 2056, 2089, 2119, 2204, 2246, 2270, 2309, 2346, 2384, 2416, 2613, 2662, 2730, 2785, 2809, 2850, 2949, 3082}
	_PrismaError_ZhCn_index_3 = [...]uint16{0, 128, 194, 239, 269, 310}
	_PrismaError_ZhCn_index_4 = [...]uint16{0, 39, 136, 178, 319, 442, 568, 653, 822, 880, 1080, 1149, 1314, 1387, 1552, 1775, 1865, 1980, 2119, 2414, 2719, 2908, 3038, 3214}
