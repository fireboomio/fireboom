...
type (
	...
	Message struct {
		...
		Payload *MultiFormatSchemaRef `json:"payload" yaml:"payload"`
		...
		Traits MessageTraits `json:"traits" yaml:"traits"`
	}
)

// MarshalYAML returns the YAML encoding of MessageRef.
func (x MessageRef) MarshalYAML() (interface{}, error) {
	if ref := x.Ref; ref != "" {
...
// MarshalJSON returns the JSON encoding of MessageRef.
func (x MessageRef) MarshalJSON() ([]byte, error) {
	if ref := x.Ref; ref != "" {
...
// UnmarshalJSON sets MessageRef to a copy of data.
func (x *MessageRef) UnmarshalJSON(data []byte) error {
...
func (x *MessageRef) JSONLookup(token string) (interface{}, error) {
	if token == "$ref" {
		return x.Ref, nil
	}
